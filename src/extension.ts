...
// 태그 변환 함수
function applyTagTransformation(
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // row/col switching
  const switchElement = vscode.commands.registerCommand(
    'decipher-niq-package.switchElement',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  const changeLabelCode = vscode.commands.registerCommand(
    'decipher-niq-package.changeLabelCode',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  registerCommand(
    context,
    'decipher-niq-package.makeStrong',
    (text) => `<b>${text}</b>`
  );
  ...
  registerCommand(
    context,
    'decipher-niq-package.makeUnderline',
    (text) => `<u>${text}</u>`
  );
  ...
  const makeCases = vscode.commands.registerCommand(
    'decipher-niq-package.makeCases',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
      ...
      try {
        for (const selection of selections) {
          ...
          lines.forEach((line: any, index: number) => {
            ...
            output += `\t<case label="r${
              index + 1
            }" cond="">${cleanedLine}</case>\n`;
          });

          if (lines.length == 0) {
            output = `\t<case label="r1" cond=""></case>\n`;
          }
  ...
  // Shortcut: Ctrl+6
  const makeLoopBlock = vscode.commands.registerCommand(
    'decipher-niq-package.makeLoopBlock',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
...
`;
  ...
  const gridColCommand = vscode.commands.registerCommand(
    'decipher-niq-package.gridCol',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  // make [loopvar: label]
  const loopLabelCommand = vscode.commands.registerCommand(
    'decipher-niq-package.loopLabel',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  const changeEntityCommand = vscode.commands.registerCommand(
    'decipher-niq-package.changeEntity',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  const makePipeCommand = vscode.commands.registerCommand(
    'decipher-niq-package.makePipe',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  // Command: ctrl+m
  // Split Context
  const splitContextCommand = vscode.commands.registerCommand(
    'decipher-niq-package.splitContext',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  const splitCommentCommand = vscode.commands.registerCommand(
    'decipher-niq-package.splitComment',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  // make looprow tag
  // ctrl+shift+6
  const makeLooprowTagCommand = vscode.commands.registerCommand(
    'decipher-niq-package.makeLooprowTag',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
  ...
  // ctrl+alt+6
  const looprowMacroCommand = vscode.commands.registerCommand(
    'decipher-niq-package.looprowMacro',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
      ...
      try {
        for (const selection of selections) {
          ...
          if (selectedText.includes('\n')) {
            vscode.window.showErrorMessage(
              `Invalid input method: ${selectedText}`
            );
            return;
          }
        ...
        for (const selection of selections) {
          ...
          for (const group of groupDict) {
            ...
            const rows = group.content.map((line) => {
              ...
              if (
                ['기타', 'other', 'others'].some((word) =>
                  content.toLowerCase().includes(word)
                ) &&
                ['구체적', '자세히', 'specify', 'specific'].some((word) =>
                  content.toLowerCase().includes(word)
                )
              ) {
                extra = ' open="1" openSize="25" randomize="0"';
              } else if (
                ['모름', '없음', 'none'].some((word) =>
                  content.toLowerCase().includes(word)
                )
              ) {
  ...
  // ctrl+shift+f7
  const makeConjointAttrs = vscode.commands.registerCommand(
    'decipher-niq-package.makeConjointAttrs',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
      ...
      try {
        for (const selection of selections) {
          ...
          legends.forEach(([legendCode, legendText]) => {
            output += `<res label="DCM_legend${legendCode}">${legendText
              .trim()
              .replace('&', '&amp;')}</res>\n`;
            levels
  ...
  // ctrl+shift+f8
  const makeMaxdiffAttrs = vscode.commands.registerCommand(
    'decipher-niq-package.makeMaxdiffAttrs',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
  ...
  // make Quota Cells
  // ctrl+shift+q
  const makeQuotaCells = vscode.commands.registerCommand(
    'decipher-niq-package.makeQuotaCells',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
      ...
      editor.edit((editBuilder) => {
        try {
          ...
          console.error(error);
          vscode.window.showErrorMessage(
            `makeQuotaCells 실행 중 오류 발생: ${error}`
          );
        }
      });
    }
  ...
  const makeRadio = vscode.commands.registerCommand(
    'decipher-niq-package.makeRadio',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
      ...
      try {
        for (const selection of selections) {
          ...
          if (
            (checkAttr(processedInput, 'row') &&
              checkAttr(processedInput, 'col')) ||
...
<suspend/>`;
          ...
          let comment = processedInput.includes('<comment>')
            ? ''
            : '<comment></comment>';
...
<radio
  ...
  <title><div class="q-name">${label
    .trim()
    .replace(/x/g, '-')}</div> ${title.trim()}</title>
  ${comment}
  ${processedInput}
</radio>
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Radio Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  // ctrl+shift+r
  const makeRating = vscode.commands.registerCommand(
    'decipher-niq-package.makeRating',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
...
<radio
  ...
  <title><div class="q-name">${label
    .trim()
    .replace(/x/g, '-')}</div> ${title.trim()}</title>${customRate}
  ${comment}
  ${processedInput}
</radio>
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Rating Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  const makeCheckbox = vscode.commands.registerCommand(
    'decipher-niq-package.makeCheckbox',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
      ...
      try {
        for (const selection of selections) {
          ...
          const comment = processedInput.includes('<comment>')
            ? ''
            : '<comment></comment>';
...
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Checkbox Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  // ctrl+shift+s
  const makeSelect = vscode.commands.registerCommand(
    'decipher-niq-package.makeSelect',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
...
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Select Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  const makeTextArea = vscode.commands.registerCommand(
    'decipher-niq-package.makeTextArea',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
...
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make TextArea Question failed: ${error.message}`
        );
      }
    }
  );
...
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Text Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  const makeNumber = vscode.commands.registerCommand(
    'decipher-niq-package.makeNumber',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
...
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Number Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  const makeFloat = vscode.commands.registerCommand(
    'decipher-niq-package.makeFloat',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
...
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Float Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  // Make Video Question
  // ctrl+shift+v
  const makeVideo = vscode.commands.registerCommand(
    'decipher-niq-package.makeVideo',
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found!');
...
<suspend/>`;
        ...
        vscode.window.showErrorMessage(
          `Make Video Question failed: ${error.message}`
        );
      }
    }
  );
  ...
  // Make Rank Columns
  const makeRankColumns = vscode.commands.registerCommand(
    'decipher-niq-package.makeRankColumns',
    () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showErrorMessage('No active editor found.');
      ...
      if (!match) {
        vscode.window.showErrorMessage(
          'Selected text is not in the correct format (rk or erk followed by a number).'
        );
        return;
      }
      ...
      if (isNaN(count) || count <= 0) {
        vscode.window.showErrorMessage(
          'The number after rk/erk must be a positive integer.'
        );
        return;
      }

      const generateColTag = (label: string, content: string) =>
        `<col label="${label}">${content}</col>`;
      let result = '';
